...
**Huge-PR fallback:** on PRs over Greptile's file limit, the tagged review may never create a check run for the new head — Greptile instead edits its existing summary comment when it finishes. If the check run has not appeared after a few attempts, poll the latest Greptile-authored issue comment instead and stop when its `updated_at` moves past the time you posted the trigger and its body carries a confidence score (rather than the "Too many files changed" notice):
...
if [ "$GREPTILE_RUNNING" = "0" ]; then
  glab mr note <MR_IID> --message "@greptile-apps review"
fi
```
